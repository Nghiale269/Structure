...
typedef struct t_danhsach ds;
///
void nhap_ds (ds danhsach )  // khoog co tham chieu van duoc
{
    // int n = danhsach.n;
    // danhsach.arr = (sv_t*)malloc(n*sizeof(sv_t));
    for (int i = 0; i< danhsach.n; i++)
    {
        printf("\n================ nhap sinh vien thu %d trong ds ===================", i+1);
        nhap_1(danhsach.arr[i]);
    }
}
...
// them 1 sinh vien
void them_sv(ds *sinhvien_ds)   // chổ  này phải có tham chiếu nó mới cập nhật n
{
    printf("\n===========================Them sinh vien=============================");
     int vt;
     int n =sinhvien_ds->n;
    ...
    if (vt > n) vt = n;
     sinhvien_ds->n++;
    realloc(sinhvien_ds->arr, sinhvien_ds->n);
    for (int i = n; i > vt ; i--)
    {
        sinhvien_ds->arr[i] = sinhvien_ds->arr[i-1];
    }
    nhap_1(sinhvien_ds->arr[vt]);
   
}
// xóa sinh vien
void xoa_sv (ds *sinhvien_ds)
{
     printf("\n===========================Xoa sinh vien=============================");
     int vt;
...
int main()
{
    ...
    scanf("%d", &sinhvien_ds.n);

   
    sinhvien_ds.arr = (sv_t*)malloc(sinhvien_ds.n*sizeof(sv_t));
